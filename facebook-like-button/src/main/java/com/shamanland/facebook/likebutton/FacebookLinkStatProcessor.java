...
public class FacebookLinkStatProcessor {
	private static final String LOG_TAG = FacebookLinkStatProcessor.class
			.getSimpleName();
	...
	protected Result getResult(String url, JSONObject json) {
		...
		result.likes = getLikes(json);
		result.comments = getComments(json);
		return result;
	}

	private long getLikes(JSONObject json) {
		return json.optLong("likes", 0);
	}
