...
public class SoundManager {
    ...
    public SoundManager(Context theContext) {
        mContext = theContext;
        mSoundPool = new SoundPool(NUM_CONCURRENT_STREAMS, AudioManager.STREAM_MUSIC, 0);
        mSoundPoolMap = new HashMap<Object, Integer>();
        mAudioManager = (AudioManager) mContext.getSystemService(Context.AUDIO_SERVICE);

        SharedPreferences preferences = PreferenceManager
                .getDefaultSharedPreferences(theContext);
        final String soundFxPrefKey = theContext.getString(R.string.pref_sound_fx_key);
        playFx = preferences.getBoolean(soundFxPrefKey, true);
        prefListener = new OnSharedPreferenceChangeListener() {
            @Override
            public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
                if (soundFxPrefKey.equals(key)) {
                    playFx = sharedPreferences.getBoolean(soundFxPrefKey, false);
                }
            }
        };
    ...
    public int playSound(Object key, boolean loop) {
        ...
        return mSoundPool.play(soundId, streamVolume, streamVolume, 1, loop ? -1 : 0, 1f);
    }
